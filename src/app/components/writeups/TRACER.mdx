export const toc = [
    {level: 1, title: "Goals", id: "goals"},
    {level: 1, title: "Parts List", id: "parts-list"},
    {level: 1, title: "Hardware", id: "hardware"},
    {level: 2, title: "Sensors", id: "sensors"},
    {level: 3, title: "Ultrasonic Sensor", id: "ultrasonic-sensor"},
    {level: 3, title: "IR Sensors", id: "ir-sensors"},
    {level: 3, title: "MPU-6050", id: "mpu-6050"},
    {level: 3, title: "LCD Display", id: "lcd-display"},
    {level: 3, title: "Battery Percentage Estimation", id: "battery-percentage-estimation"},
    {level: 1, title: "The next step: Connecting the hardware side to a proper computer", id: "the-next-step-connecting-the-hardware-side-to-a-proper-computer"},
    {level: 2, title: "The Bluetooth Module", id: "the-bluetooth-module"},
    {level: 2, title: "Switching to Wi-Fi", id: "switching-to-wi-fi"},
    {level: 2, title: "Building the architecture", id: "building-the-architecture"},
    {level: 2, title: "The final architecture", id: "the-final-architecture"},
    {level: 3, title: "Raspberry-pi layer", id: "raspberry-pi-layer"},
    {level: 3, title: "Arduino layer", id: "arduino-layer"},
    {level: 3, title: "Web interface layer", id: "web-interface-layer"},
    {level: 3, title: "Web dashboard", id: "web-dashboard"},
    {level: 3, title: "Web backend", id: "web-backend"},
    {level: 2, title: "Implementing the architecture", id: "implementing-the-architecture"},
    {level: 3, title: "Sending and parsing commands", id: "sending-and-parsing-commands"},
    {level: 3, title: "Sending data", id: "sending-data"},
    {level: 3, title: "Major serial issue", id: "major-serial-issue"},
    {level: 3, title: "Request-Response Mechanism", id: "request-response-mechanism"},
    {level: 2, title: "The Robot Class", id: "the-robot-class"},
];
...
<Info>
  {" "}
  These directions depend on how the motors are wired, so you may need to
  reverse the direction of one of the motors in your code while testing.{" "}
</Info>
...
<Warning>
  {" "}
  Make sure to enable output on the pins used here in your code, otherwise the
...
Since I used 2 batteries in series, the maximum voltage is 8.4V (2 _ 4.2V), and the minimum voltage is 6.0V (2 _ 3.0V).
...
I used a 10k and 3k resistor to create a voltage divider, which gives us a ratio of 3 / 13. This means that the maximum voltage we can read is 5V _ (3 / 13) = 1.15V, and the minimum voltage is 0V _ (3 / 13) = 0V. (More info on voltage dividers below in the bluetooth section)
...
<Info>
  The Raspberry Pi is powered by a portable anker power bank. This is the safest
  option compared to using batteries directly
</Info>
...
<Info>
  The raspberry pi is formatted with a headless version of Raspberry Pi OS,
  which is a lightweight version of the operating system without a graphical
  user interface. This allows for better performance and lower resource usage,
  as the pi is only used for running the FastAPI server and communicating with
  the arduino.
</Info>
...
<Info>
  {" "}
  With the SoftwareSerial and bluetooth bottleneck gone, I could use much higher
  baud rates, so I decided to use a baud rate of 115200 bits/second. This
  doesn't sound like much (only 11.5 kB/s), but with the 24 byte sensor packet,
  this means we can send a packet every 2 ms, which is more than enough for
...
<Info>
  I put this at the very end of the loop because I noticed sending sensor data
  right after setting the motors would result in nothing being sent. I think
  this is because the motors take some time to start moving, and the arduino is
  busy processing the motor command, so it doesn't have time to send the sensor
  data. So I just put it at the end of the loop to ensure that all commands are
  fully processed before sending sensor data.
</Info>
...
<Info>
  I send joystick update events to the frontend so that the UI can update the
  joystick position and display the current state of the robot. This is useful
  for debugging and monitoring the robot's state.
</Info>
...
<Info>
  {" "}
  I add descriptions to the fields so that the LLM can understand what each
  field means. This is useful for generating the JSON schema and for the LLM to
  understand what data it needs to return.
</Info>
...
<Info>
  Notice that I run the command sequence in a task. This prevents it from
  blocking any other process that sends motor commands, and this allows the
  obstacle avoidance and manual control to be active even while the AI commands
  are executing.
</Info>
...
<iframe className="rounded-xl mt-8 mb-4" width="800" height="450" src="https://www.youtube.com/embed/NjCj3DIxfyM?si=XZgnj33kS7oclzKd" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
