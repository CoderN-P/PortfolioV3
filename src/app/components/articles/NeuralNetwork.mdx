...
export const toc = [
    { title: "Minimizing the error function", id: "minimizing-the-error-function", level: 1 },
    { title: "Solving the error function", id: "solving-the-error-function", level: 2 },
    { title: "Real world example", id: "real-world-example", level:2 },
    { title: "Generalizing to multiple features", id: "generalizing-to-multiple-features", level: 1 },
    { title: "Multiple outputs", id: "multiple-outputs", level: 2 },
    { title: "Neural Networks", id: "neural-networks", level: 1 },
    { title: "Activation Functions", id: "activation-functions", level: 2 },
    { title: "The forward pass", id: "the-forward-pass", level: 1 },
    { title: "Training the neural network", id: "training-the-neural-network", level: 1 },
    { title: "Gradients and Jacobians", id: "gradients-and-jacobians", level: 2 },
    { title: "Backpropagation + Gradient Descent", id: "backpropagation-plus-gradient-descent", level: 2 },
    { title: "The trick", id: "the-trick", level: 3 },
    { title: "The outer product", id: "the-outer product", level: 3 },
    { title: "The bias", id: "the-bias", level: 2 },
    { title: "Continuing Backwards", id: "continuing-backwards", level: 2 },
    { title: "Backpropagation Algorithm", id: "backpropagation-algorithm", level: 2 },
    { title: "Iterating over the dataset", id: "iterating-over-the-dataset", level: 2 },
    { title: "Overfitting and Regularization", id: "overfitting-and-regularization", level: 2 },
    { title: "Summary", id: "summary", level: 1 },
];
...
<Info>
  Often the input vector is normalized or standardized before being fed into the
  network to ensure that all features are on a similar scale. This helps with
...
The Jacobian of $z_2$ with respect to $a_1$ is just the weight matrix $W_2$, because each element of $z_2$ changes by the same amount as the corresponding element of $a_1$ scaled by the weights.

We know that the shape of $W_2$ is (m, n), and the shape of $\nabla_{z_2} L$ is (m, 1). Therefore, we can't multiply them directly because the inner dimensions don't match (m != 1). However, if we transpose $W_2$ to have shape (n, m), then the inner dimensions match (1 == 1) and we can do the multiplication to get a vector of shape (n, 1).
...
layers = [ (W1, b1, f1), (W2, b2, f2), ..., (Wn, bn, fn) ]  # List of layers with weights, biases, and activation functions

# forward pass
layers_output = []  # To store outputs of each layer
input = x          # Initial input to the network

for (W, b, f) in layers:
    z = W * input + b  # Linear transformation
    a = f(z)           # Activation function
    layers_output.append((z, a))  # Store pre-activation and post-activation outputs
    input = a         # Output becomes input for next layer

a2 = input  # Final output after last layer

# Compute loss
L = loss(a2, y)  # Loss function comparing output to true labels
# Backward pass
# Compute gradients for output layer

loss_grad = loss_derivative(a2, y)  # Gradient of loss with respect to output

for index in range(len(layers)-1, -1, -1):
    W, b, f = layers[index]
    z = layers_output[index][0]  # Pre-activation output of current layer
    f_jacobian = activation_jacobian(f, z)  # Jacobian of activation function

    # Gradient with respect to pre-activation output
    grad_z = loss_grad * f_jacobian

    # Gradients with respect to weights and biases
    grad_W = grad_z * input.T  # Outer product
    grad_b = grad_z             # Gradient with respect to bias
    ...
    loss_grad = grad_z * W.T  # Backpropagate through weights
